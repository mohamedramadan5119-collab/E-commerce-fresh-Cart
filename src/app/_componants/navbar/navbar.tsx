    ...
    <nav className="bg-gray-200 py-2 sticky top-0 z-50 shadow-sm">
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        ...
        <Link href="/" className="flex items-center space-x-3 rtl:space-x-reverse z-50">
          <Image src={logo} alt="Logo" width={40} height={20} className="rounded-full mix-blend-multiply" />
          <span className="self-center text-xl font-bold whitespace-nowrap text-green-600">Fresh Cart</span>
        </Link>
        ...
        <div className="flex items-center md:order-2 space-x-2 md:space-x-4 rtl:space-x-reverse z-50">
          {status === 'authenticated' && (
             <div className="flex items-center space-x-3 md:space-x-5">
                ...
                <Link href='/wishlist' className="relative text-gray-600 hover:text-red-500 transition-all">
                  {wishlistCount > 0 && (
                ...
                <Link href='/cart' className="relative text-gray-600 hover:text-green-600 transition-all">
          ...
          <button onClick={toggleMenu} type="button" className="inline-flex items-center p-2 w-10 h-10 justify-center text-sm rounded-lg md:hidden focus:outline-none focus:ring-2 focus:ring-gray-400">
        ...
        <div className={`${isOpen ? 'block' : 'hidden'} w-full md:flex md:w-auto md:items-center md:justify-between flex-grow md:ml-10`} id="navbar-default">
          ...
          <div className="hidden md:flex items-center space-x-4 rtl:space-x-reverse ml-auto">
             {status === 'authenticated' ? (
                <span className="text-gray-700 text-md me-5">HI, <span className='text-green-600 text-[18px] font-bold uppercase'>{session?.user?.name}</span></span>
             ) : (
                <div className="flex space-x-4">
                  {authPath.map((elem) => (
                    <Link key={elem.content} href={elem.href} className="text-gray-700 hover:text-green-600 font-medium">
                      {elem.content}
                    </Link>
                  ))}
                </div>
