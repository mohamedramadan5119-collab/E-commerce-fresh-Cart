...
// تحديد الأنواع (Types) عشان نتخطى مشاكل الـ Build في Vercel
interface OrderItem {
  count: number;
  price: number;
  product: {
...
export default function AllOrders() {
  ...
  // استخراج الـ ID مع التأكد إنه موجود
  ...
  const { data: orders, isLoading, isError } = useQuery<Order[]>({
    ...
    queryFn: async () => {
      ...
      return result; // الـ API ده بيرجع مصفوفة مباشرة
    },
    enabled: !!userId, 
  });

  // شاشة التحميل بشكل احترافي
  if (isLoading) return (
  ...
  if (isError || !orders || orders.length === 0) return (
    <div className="min-h-screen flex flex-col justify-center items-center text-gray-500 bg-[#f8f9fa] px-4 text-center">
      <div className="bg-white p-8 rounded-full shadow-sm mb-6">
        <ShoppingBag size={80} className="text-gray-200" />
      </div>
      <p className="text-2xl font-bold text-gray-800">No orders found yet</p>
      <p className="text-sm mt-2 max-w-xs">Your shopping history is empty. Start exploring our products and make your first order!</p>
    </div>
  );
    ...
    <div className="bg-[#f8f9fa] min-h-screen py-10">
      <div className="container mx-auto px-4 max-w-5xl">
        ...
        <div className="space-y-8">
          {orders.map((order) => (
            <div key={order._id} className="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow duration-300">
               {/* Header الطلب */}
               <div className="p-6 border-b border-gray-50 bg-gray-50/30 flex flex-wrap justify-between items-center gap-4">
                <div className="flex gap-8">
                  <div>
                    <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1">Order Ref</p>
                    <p className="text-sm font-mono font-bold text-gray-700 bg-white px-2 py-1 rounded-md border border-gray-100">
                      #{order._id.slice(-8).toUpperCase()}
                    </p>
                  </div>
                  <div>
                    <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1">Placed On</p>
                ...
                <div className="flex items-center gap-4">
                  <div className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-[12px] font-bold ${order.isPaid ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xl font-black text-[#0aad0a]">{order.totalOrderPrice.toLocaleString()} <span className="text-xs">EGP</span></p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6 bg-white">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {order.cartItems.map((item, idx) => (
                    <div key={idx} className="flex gap-4 p-4 rounded-2xl border border-gray-50 bg-[#fbfbfb] hover:bg-white hover:border-[#0aad0a]/20 transition-all duration-200 group">
                      ...
                      <div className="flex flex-col justify-center flex-1">
                        <h4 className="text-sm font-bold text-gray-800 line-clamp-2 leading-snug mb-1">{item.product.title}</h4>
                        <div className="flex items-center justify-between">
                           <p className="text-xs text-gray-500">Qty: <span className="text-gray-900 font-bold">{item.count}</span></p>
                           <p className="text-sm font-bold text-gray-700">{item.price} <span className="text-[10px]">EGP</span></p>
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              <div className="px-6 py-3 bg-gray-50/50 flex justify-between items-center text-[11px] text-gray-400 font-medium">
                <div className="flex items-center gap-1">
                  <MapPin size={12} />
                  <span>Delivering to: {order.shippingAddress?.city || 'Default Address'}</span>
                </div>
